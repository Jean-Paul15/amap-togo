...
export async function middleware(request: NextRequest) {
  ...
  if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY) {
    ...
    return new NextResponse(
      JSON.stringify({
        ...
        message: 'Supabase environment variables are not configured. Please add NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY to your Vercel project settings.'
      }),
      {
        status: 500,
  ...
  try {
    ...
    // En cas d'erreur dans le middleware, logger et rediriger vers le front
